...
class AppConfig:
    ...
    def validate_sarima_parameters(self, order: list, seasonal_order: list) -> bool:
        ...
        try:
            ...
            if not (isinstance(order, list) and len(order) == 3 and all(isinstance(x, int) for x in order)):
                return False
            if not (isinstance(seasonal_order, list) and len(seasonal_order) == 4 and all(isinstance(x, int) for x in seasonal_order)):
                return False
