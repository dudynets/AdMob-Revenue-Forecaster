...
class AdMobAPIClient:
            ...
            if not creds or not creds.valid:
                ...
                if not creds:
                    ...
                    if not Path(credentials_file).exists():
                        self.logger.error("OAuth2 credentials file not found. Please add your Google OAuth2 credentials.")
                        return False
    ...
    def get_apps(self, account_id: str) -> List[Dict]:
        ...
        try:
            request = self.service.accounts().apps().list(parent=f"accounts/{account_id}")
            response = request.execute()
    ...
    def generate_network_report(self, account_id: str, start_date: str, end_date: str) -> pd.DataFrame:
        """Generate network report for revenue data"""
        try:
            ...
            self.logger.error(f"HTTP error generating network report: {e}")
            return pd.DataFrame()
        except Exception as e:
    ...
    def get_revenue_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            # Generate report
            df = self.generate_network_report(account_id, start_date, end_date)
            
            if df.empty:
                self.logger.warning("No revenue data found for the specified date range")
                return pd.DataFrame()
    ...
    def fill_missing_dates(self, df: pd.DataFrame, start_date: str, end_date: str) -> pd.DataFrame:
        """Fill missing dates with 0 revenue"""
        try:
        ...
        try:
            ...
            if self.config.is_cache_valid(cache_file):
                ...
                if not df.empty:
                    ...
                    df.set_index('date', inplace=True)
                    self.logger.info(f"Using cached revenue data from {start_date} to {end_date}")
                    return df
    ...
    def cache_data(self, df: pd.DataFrame, start_date: str, end_date: str):
        ...
        try:
            ...
            self.logger.info(f"Cached revenue data for {start_date} to {end_date}")
            
        except Exception as e:
            self.logger.error(f"Error caching data: {e}")
    
    def fetch_revenue_data(self, start_date: str, end_date: str, use_cache: bool = True) -> pd.DataFrame:
