...
class DataProcessor:
    ...
    def validate_data(self, data: pd.DataFrame) -> Dict[str, bool]:
        ...
        try:
            ...
            if len(data) < 30:
                self.logger.warning(f"Insufficient data: {len(data)} days (minimum 30 required)")
            else:
                validation_results['sufficient_data'] = True
            
            # Check for excessive gaps (more than 7 consecutive days with 0 revenue)
            consecutive_zeros = (data['revenue'] == 0).astype(int).groupby(
                (data['revenue'] != 0).cumsum()
            ).sum()
            
            if consecutive_zeros.max() > 7:
                self.logger.warning(f"Found {consecutive_zeros.max()} consecutive days with zero revenue")
            else:
            ...
            if max_revenue > mean_revenue * 100:  # Max > 100x mean
                self.logger.warning(f"Potentially unreasonable max revenue: ${max_revenue:.2f}")
            else:
            ...
            self.logger.info(f"Data validation: {passed_checks}/{total_checks} checks passed")
    ...
    def clean_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'revenue' in cleaned_data.columns:
                ...
                if outliers.any():
                    self.logger.warning(f"Found {outliers.sum()} outliers, capping at ${outlier_threshold:.2f}")
    ...
    def add_derived_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            data['revenue_std_7'] = data['revenue'].rolling(window=7, min_periods=1).std()
            data['revenue_std_30'] = data['revenue'].rolling(window=30, min_periods=1).std()
    ...
    def resample_data(self, data: pd.DataFrame, frequency: str = 'D') -> pd.DataFrame:
        ...
        try:
            ...
            existing_agg_funcs = {col: func for col, func in agg_funcs.items() if col in data.columns}
            ...
            self.logger.info(f"Data resampled to {frequency} frequency: {len(resampled)} records")
            return resampled
    ...
    def split_data(self, data: pd.DataFrame, test_size: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            self.logger.info(f"Data split: {len(train_data)} training, {len(test_data)} testing")
    ...
    def get_data_summary(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            data_quality = {
                ...
                'negative_values': (data['revenue'] < 0).sum() if 'revenue' in data.columns else 0,
                'completeness_pct': (len(data) / self._get_expected_days(data.index.min(), data.index.max())) * 100
            }
    ...
    def export_data(self, data: pd.DataFrame, filepath: str, format: str = 'csv') -> bool:
        """Export data to file"""
        try:
            if data.empty:
