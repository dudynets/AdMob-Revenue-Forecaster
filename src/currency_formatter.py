...
class CurrencyFormatter:
    ...
    def fetch_currencies(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.info(
                    f"Currency list updated successfully. {len(self.currencies)} currencies loaded."
                )
                return True
            else:
                logging.warning(
                    f"Primary currencies API failed with status {response.status_code}"
                )
    ...
    def _try_fallback_currencies_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.currencies = response.json()
                logging.info(
                    f"Currency list updated using fallback API. {len(self.currencies)} currencies loaded."
                )
                return True
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "usd" in data:
                    ...
                    logging.info(
                        f"Exchange rates updated successfully. {len(self.rates)} currencies loaded."
                    )
                    return True
                else:
                ...
                logging.warning(
                    f"Primary API failed with status {response.status_code}"
                )
                return self._try_fallback_api()
    ...
    def initialize(self):
        ...
        logging.info(
            f"Currency formatter initialized. Local currency: {self.get_local_currency()}"
        )
