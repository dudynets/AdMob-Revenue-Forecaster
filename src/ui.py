...
from PyQt6.QtWidgets import (
    ...
    QFormLayout, QGridLayout, QSplitter, QMessageBox,
    QFileDialog, QStatusBar, QMenuBar, QScrollArea
)
    ...
    def run(self):
        try:
            ...
            if not self.api_client.authenticate():
                self.error_occurred.emit("Authentication failed. Please check your OAuth2 credentials and try again.")
                return
            ...
            if data.empty:
                self.error_occurred.emit("No data retrieved from AdMob API. Please check your date range and account access.")
            else:
                self.status_update.emit(f"Successfully fetched {len(data)} days of data")
            ...
            if "authentication" in error_msg.lower():
                ...
                self.error_occurred.emit(f"Permission error: {error_msg}. Please check your AdMob account access.")
            else:
...
class ForecastingThread(QThread):
    ...
    def __init__(self, data, config, forecast_days, run_backtest=True, backtest_days=90):
        super().__init__()
        self.data = data
        self.config = config
    ...
    def run(self):
        try:
            ...
            if self.run_backtest:
                ...
                backtest_results = forecaster.backtest(self.data, test_months=test_months)
...
5. Test the connection
        ...
        instructions_label.setStyleSheet("QLabel { font-size: 11px; color: #888; margin-bottom: 10px; }")
        ...
        self.credentials_status_label.setStyleSheet("QLabel { color: #888; font-style: italic; }")
        ...
        self.customer_id_edit = QLineEdit(self.config.get('api_settings.customer_id', ''))
        self.customer_id_edit.setPlaceholderText("pub-1234567890123456 (from AdMob console)")
        ...
        self.forecast_days_spin.setValue(self.config.get('forecast_settings.default_forecast_days', 30))
        self.forecast_days_spin.setSuffix(" days")

        self.backtest_days_spin = QSpinBox()
        self.backtest_days_spin.setRange(1, 365 * 10) # 10 years
        self.backtest_days_spin.setValue(self.config.get('forecast_settings.default_backtest_days', 90))
        self.backtest_days_spin.setSuffix(" days")

        # SARIMA Parameters
        sarima_order = self.config.get('forecast_settings.sarima_order', [1, 1, 1])
        seasonal_order = self.config.get('forecast_settings.seasonal_order', [1, 1, 1, 7])
...
• p (AutoRegressive): Number of lag observations in the model (0-5)
  - Higher values capture more historical patterns
  - Start with 1-2 for daily revenue data

• d (Differencing): Degree of differencing to make data stationary (0-2)
  - 0 = data is already stationary
  - 1 = first difference (most common)
  - 2 = second difference (rarely needed)

• q (Moving Average): Size of moving average window (0-5)
  - Captures error correction from previous periods
  - Start with 1-2 for daily revenue data

• P, D, Q (Seasonal): Same as p, d, q but for seasonal patterns
  - Use when data has recurring patterns (weekly, monthly)
  - Seasonal Period: 7 for weekly, 30 for monthly patterns

Default values (1,1,1)(1,1,1,7) work well for daily revenue with weekly patterns.
        """)
        explanation_label.setWordWrap(True)
        explanation_label.setStyleSheet("QLabel { font-size: 11px; color: #888; }")
        ...
        self.auto_refresh_check.setChecked(self.config.get('ui_settings.auto_refresh', True))
        
        self.refresh_interval_spin = QSpinBox()
        self.refresh_interval_spin.setRange(300, 86400)  # 5 minutes to 24 hours
        self.refresh_interval_spin.setValue(self.config.get('ui_settings.refresh_interval', 3600))
        ...
        self.exchange_rate_label.setStyleSheet("QLabel { color: #666; font-size: 12px; }")
    ...
    def upload_credentials_file(self):
        ...
        if file_path:
            try:
                ...
                if not is_valid and 'client_id' in credentials and 'client_secret' in credentials:
                    is_valid = True
    ...
    def update_credentials_status(self):
        ...
        if self.config.credentials_file.exists():
            ...
            self.credentials_status_label.setStyleSheet("QLabel { color: #28a745; font-weight: bold; }")
        else:
    ...
    def update_exchange_rate_display(self):
        ...
        try:
            ...
            if current_currency == "USD":
                ...
                exchange_rate = self.currency_formatter.get_current_exchange_rate(current_currency)
                
                if exchange_rate is not None:
                    self.exchange_rate_label.setText(f"1.00 USD = {exchange_rate:,.2f} {current_currency}")
                    self.exchange_rate_label.setStyleSheet("QLabel { color: #28a745; font-size: 12px; font-weight: bold; }")
                else:
                    self.exchange_rate_label.setText("Exchange rate not available")
                    self.exchange_rate_label.setStyleSheet("QLabel { color: #dc3545; font-size: 12px; font-style: italic; }")
    ...
    def test_connection(self):
        ...
        try:
            ...
            if api_client.authenticate():
                QMessageBox.information(self, "Success", "Successfully connected to AdMob API!")
            else:
                QMessageBox.warning(self, "Error", "Failed to connect to AdMob API. Please check your credentials.")
    ...
    def save_settings(self):
        ...
        try:
            # API Settings - only save customer_id now
            self.config.set('api_settings.customer_id', self.customer_id_edit.text())
            
            # Forecast Settings
            self.config.set('forecast_settings.default_forecast_days', self.forecast_days_spin.value())
            self.config.set('forecast_settings.default_backtest_days', self.backtest_days_spin.value())
            self.config.set('forecast_settings.sarima_order', [
                self.p_spin.value(), self.d_spin.value(), self.q_spin.value()
            ])
            self.config.set('forecast_settings.seasonal_order', [
                self.sp_spin.value(), self.sd_spin.value(), 
                self.sq_spin.value(), self.seasonal_period_spin.value()
            ])
            
            # Application Settings
            self.config.set('ui_settings.auto_refresh', self.auto_refresh_check.isChecked())
            self.config.set('ui_settings.refresh_interval', self.refresh_interval_spin.value())
...
class DataTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.start_date_edit = QDateEdit(QDate.fromString(min_date.strftime('%Y-%m-%d'), 'yyyy-MM-dd'))
        self.start_date_edit.setCalendarPopup(True)
        # Remove minimum date restriction - allow any date
        self.start_date_edit.setMaximumDate(QDate.fromString(max_date.strftime('%Y-%m-%d'), 'yyyy-MM-dd'))
        
        self.end_date_edit = QDateEdit(QDate.fromString(max_date.strftime('%Y-%m-%d'), 'yyyy-MM-dd'))
        self.end_date_edit.setCalendarPopup(True)
        # Remove minimum date restriction - allow any date
        self.end_date_edit.setMaximumDate(QDate.fromString(max_date.strftime('%Y-%m-%d'), 'yyyy-MM-dd'))
    ...
    def fetch_data(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to start data fetching: {str(e)}")
    ...
    def reset_dates(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Success", "Date range reset to default values!")
    ...
    def update_status(self, message):
        ...
        # Could be connected to a status bar if needed
        pass
    ...
    def on_data_ready(self, data):
        ...
        try:
            ...
            self.current_data = self.data_processor.clean_data(data)
    ...
    def update_data_summary(self):
        ...
        try:
            ...
            total_revenue = self.currency_formatter.format_currency(revenue_stats.get('total_revenue', 0))
            avg_daily_revenue = self.currency_formatter.format_currency(revenue_stats.get('average_daily_revenue', 0))
            median_daily_revenue = self.currency_formatter.format_currency(revenue_stats.get('median_daily_revenue', 0))
            max_daily_revenue = self.currency_formatter.format_currency(revenue_stats.get('max_daily_revenue', 0))
            min_daily_revenue = self.currency_formatter.format_currency(revenue_stats.get('min_daily_revenue', 0))
...
- Median Daily Revenue: {median_daily_revenue}
- Max Daily Revenue: {max_daily_revenue}
- Min Daily Revenue: {min_daily_revenue}
- Zero Revenue Days: {summary.get('revenue_stats', {}).get('zero_revenue_days', 0)}

Data Quality:
- Missing Values: {summary.get('data_quality', {}).get('missing_values', 0)}
- Duplicate Dates: {summary.get('data_quality', {}).get('duplicate_dates', 0)}
- Completeness: {summary.get('data_quality', {}).get('completeness_pct', 0):.1f}%
"""
    ...
    def export_data(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self, "Export Data", "", "CSV Files (*.csv);;JSON Files (*.json);;Excel Files (*.xlsx)"
            )
            
            if file_path:
                file_format = file_path.split('.')[-1].lower()
                
                if self.data_processor.export_data(self.current_data, file_path, file_format):
    ...
    def import_data(self):
        ...
        try:
            file_path, _ = QFileDialog.getOpenFileName(
                self, "Import Data", "", "CSV Files (*.csv);;JSON Files (*.json);;Excel Files (*.xlsx)"
            )
...
class ForecastTab(QWidget):
    ...
    def run_forecast(self):
        ...
        try:
            ...
            if run_backtest:
                ...
                min_training_days = backtest_days + 30  # Need at least 30 days for training
            ...
            QMessageBox.critical(self, "Error", f"Failed to start forecasting: {str(e)}")
    ...
    def on_forecast_ready(self, forecast_data, results):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to process forecast results: {str(e)}")
...
- Model AIC: {format_numeric(diagnostics.get('aic'))}
- Model BIC: {format_numeric(diagnostics.get('bic'))}
- Log Likelihood: {format_numeric(diagnostics.get('log_likelihood'))}
- Model Order: {diagnostics.get('model_order', 'N/A')}
- Seasonal Order: {diagnostics.get('seasonal_order', 'N/A')}

Model Parameters Explanation:
• AIC (Akaike Information Criterion): Lower values indicate better model fit
• BIC (Bayesian Information Criterion): Lower values indicate better model fit  
• Log Likelihood: Higher values indicate better model fit
• Model Order (p,d,q): AutoRegressive, Differencing, Moving Average terms
• Seasonal Order (P,D,Q,s): Seasonal AR, Differencing, MA terms, seasonal period
"""
            ...
            if 'backtest' in self.forecast_results:
                ...
                results_text += f"""
Backtest Results:
- Test Period: {backtest.get('test_period', 'N/A')}
- RMSE: {format_numeric(metrics.get('rmse'))}
- MAE: {format_numeric(metrics.get('mae'))}
- MAPE: {format_numeric(metrics.get('mape'))}%
...
• MAPE (Mean Absolute Percentage Error): Lower percentages indicate better accuracy
"""
...
class VisualizationTab(QWidget):
    ...
    def check_and_update_chart(self):
        ...
        try:
            ...
            data_hash = hash(str(current_data.values.tobytes() if not current_data.empty else ""))
            forecast_hash = hash(str(forecast_results))
            
            # Check if data or forecast has changed
            if (data_hash != self.last_data_hash or 
    ...
    def update_chart(self):
        ...
        try:
            ...
            customdata = current_data['revenue_cumulative'] if 'revenue_cumulative' in current_data.columns else current_data['revenue'].cumsum()
            ...
            if local_currency == "USD":
                ...
                exchange_rate = self.currency_formatter.get_current_exchange_rate(local_currency)
                if exchange_rate is not None:
            ...
            if local_currency == "USD":
                ...
                if exchange_rate is not None:
                    ...
                    custom_data_array = list(zip(
                        daily_converted,
                        customdata,
                        customdata_converted
                    ))
            ...
            if forecast_results and 'forecast_data' in forecast_results:
                ...
                if not forecast_data.empty:
                    ...
                    forecast_cumulative = last_cumulative + forecast_data['forecast'].cumsum()
                    ...
                    if local_currency == "USD":
                        ...
                        if exchange_rate is not None:
                            ...
                            forecast_daily_converted = forecast_data['forecast'] * exchange_rate
                            forecast_cumulative_converted = forecast_cumulative * exchange_rate
                            
                            # Create array of custom data: [daily_local, cumulative_usd, cumulative_local]
                            forecast_custom_data_array = list(zip(
                    ...
                    if self.show_confidence_intervals and 'upper_ci' in forecast_data.columns and 'lower_ci' in forecast_data.columns:
                        fig.add_trace(go.Scatter(
                            x=forecast_data.index,
            ...
            if self.show_backtest and forecast_results and 'backtest' in forecast_results:
                ...
                if 'forecast_data' in backtest:
                    ...
                    if not backtest_forecast.empty:
                        ...
                        if backtest_start_date in customdata.index:
                            backtest_start_cumulative = customdata.loc[:backtest_start_date].iloc[-2] if len(customdata.loc[:backtest_start_date]) > 1 else 0
                            ...
                            if backtest_start_date > actual_end_date:
                                ...
                                backtest_start_cumulative = customdata[customdata.index <= backtest_start_date].iloc[-1] if len(customdata[customdata.index <= backtest_start_date]) > 0 else 0
                        
                        backtest_cumulative = backtest_start_cumulative + backtest_forecast['forecast'].cumsum()
                        ...
                        if local_currency == "USD":
                            ...
                            if exchange_rate is not None:
                                ...
                                backtest_daily_converted = backtest_forecast['forecast'] * exchange_rate
                                backtest_cumulative_converted = backtest_cumulative * exchange_rate
                                
                                # Create array of custom data: [daily_local, cumulative_usd, cumulative_local]
                                backtest_custom_data_array = list(zip(
            ...
            if WEB_ENGINE_AVAILABLE:
                ...
                total_revenue = self.currency_formatter.format_currency(current_data['revenue'].sum())
                avg_daily_revenue = self.currency_formatter.format_currency(current_data['revenue'].mean())
                max_daily_revenue = self.currency_formatter.format_currency(current_data['revenue'].max())
                ...
                if forecast_results and 'forecast_data' in forecast_results:
                    forecast_data = forecast_results['forecast_data']
                    if not forecast_data.empty:
                        predicted_avg_daily = self.currency_formatter.format_currency(forecast_data['forecast'].mean())
                        predicted_total = self.currency_formatter.format_currency(forecast_data['forecast'].sum())
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.forecast_tab = ForecastTab(self.config, self.data_tab, self.currency_formatter)
        self.visualization_tab = VisualizationTab(self.config, self.data_tab, self.forecast_tab, self.currency_formatter)
    ...
    def update_tab_states(self):
        ...
        self.tab_widget.setTabEnabled(self.visualization_tab_index, has_data and has_forecast)
        ...
        if has_data and has_forecast:
            ...
            self.tab_widget.setTabText(self.visualization_tab_index, "Visualization (No Forecast)")
    ...
    def setup_timers(self):
        ...
        if self.config.get('ui_settings.auto_refresh', True):
            interval = self.config.get('ui_settings.refresh_interval', 3600) * 1000  # Convert to milliseconds
    ...
    def show_about(self):
        """Show about dialog"""
        QMessageBox.about(self, "About AdMob Revenue Forecaster",
                         "AdMob Revenue Forecaster v1.0\n\n"
